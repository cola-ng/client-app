//! Role Play Screen - Stage selection and immersive role-play learning
//!
...
//! - Today's featured stages (dynamic from API)
...
/// Stage category for filtering (UI only, not from backend)
...
impl Widget for Scenes {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        for (widget_id, category) in [
            (ids!(scene_list.search_bar.search_row.filter_chips.filter_all), StageCategory::All),
            (ids!(scene_list.search_bar.search_row.filter_chips.filter_daily), StageCategory::Daily),
            (ids!(scene_list.search_bar.search_row.filter_chips.filter_travel), StageCategory::Travel),
            (ids!(scene_list.search_bar.search_row.filter_chips.filter_business), StageCategory::Business),
        ] {
        ...
        let mut stages_result: Option<Result<Vec<Stage>, String>> = None;

        if let Some(rx) = &self.fetch_rx {
            while let Ok(result) = rx.try_recv() {
                match result {
                    FetchResult::Stages(r) => stages_result = Some(r),
                }
            }
        }

        // Process stages result
        if let Some(result) = stages_result {
            match result {
                Ok(stages) => {
                    self.stages = stages;
                    self.stages_loading = false;
                    self.filter_stages();
                    self.view
...
impl Scenes {
    /// Filter stages based on search query (category filtering is UI-only since backend doesn't have category)
    fn filter_stages(&mut self) {
        let query = self.search_query.to_lowercase();

        self.filtered_stages = self.stages.iter()
            .filter(|stage| {
                // Category filter - for now just show all since backend doesn't have category
                // In the future, this could be implemented with client-side logic
                if self.active_category != StageCategory::All {
                    // For now, show all regardless of category filter
                    // return false;
                }
                ...
                let name_zh_matches = stage.name_zh.to_lowercase().contains(&query);
                let name_en_matches = stage.name_en.to_lowercase().contains(&query);
    ...
    /// Update stage cards with filtered data
    fn update_stage_cards(&mut self, cx: &mut Cx) {
        ...
        for (i, card_id) in card_ids.iter().enumerate() {
            ...
            if i < self.filtered_stages.len() {
                let stage = &self.filtered_stages[i];

                // Show card
                card.set_visible(cx, true);

                // Set icon
                let icon = stage.icon_emoji.as_deref().unwrap_or("🎭");
                card.label(ids!(icon)).set_text(cx, icon);

                // Set Chinese title
                card.label(ids!(title)).set_text(cx, &stage.name_zh);

                // Set English title
                card.label(ids!(title_en)).set_text(cx, &stage.name_en);

                // Set difficulty badge based on numeric difficulty level
                // difficulty levels: 1-2 = beginner, 3 = intermediate, 4+ = advanced
                let difficulty = stage.difficulty.unwrap_or(2);
                let (stars, badge_color) = if difficulty <= 2 {
                    ("⭐", 0.0)  // green - beginner
                } else if difficulty <= 3 {
                    ("⭐⭐", 1.0)  // amber - intermediate
                } else {
                    ("⭐⭐⭐", 2.0)  // red - advanced
                };
            ...
            rt.block_on(async {
                if let Some(api) = get_asset_api() {
                    if let Ok(client) = api.read() {
                        // Fetch stages
                        let stages_result = client.list_stages().await;
                        let _ = tx.send(FetchResult::Stages(stages_result));
                    }
                }
            });
        });
