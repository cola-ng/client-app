...
fn send_log(node: &mut DoraNode, level: LogLevel, config_level: LogLevel, message: &str) {
    ...
    let node_name =
        std::env::var("DORA_NODE_NAME").unwrap_or_else(|_| "conference-controller".to_string());
...
impl ConferenceController {
    fn new(pattern: String, node: &mut DoraNode, log_level: LogLevel) -> Result<Self> {
        ...
        send_log(
            ...
            log_level,
            &format!(
                "✅ Policy configured with participants: {:?}",
                policy.get_participants()
            ),
        );
        ...
        send_log(
            ...
            LogLevel::Info,
            log_level,
            &format!("🔄 Participant name mapping: {:?}", participant_name_map),
        );
        ...
        send_log(
            ...
            log_level,
            &format!(
                "🏷️ Starting with enhanced question_id: {} ({})",
                initial_enhanced_id,
                enhanced_id_debug_string(initial_enhanced_id)
            ),
        );
    ...
    fn handle_participant_input(
        ...
        if session_status == Some("reset") {
            send_log(
                ...
                self.log_level,
                &format!(
                    "🔄 RESET SIGNAL from {} - discarding ALL inputs",
                    participant_id
                ),
            );
        ...
        if is_error {
            send_log(
                ...
                self.log_level,
                &format!(
                    "❌ {} had an error - proceeding to next speaker",
                    participant_id
                ),
            );
        ...
        self.participant_inputs
            .insert(participant_id.to_string(), input);
    ...
    fn handle_session_start(
        ...
        send_log(
            ...
            &format!(
                "🎬 Session start: {} ({})",
                question_id,
                enhanced_id_debug_string(question_id)
            ),
        );
        ...
        if self.waiting_for_session_start == Some(question_id) {
            ...
            send_log(
                ...
                log_level,
                &format!(
                    "📝 Session start for question_id={} (not waiting for this one)",
                    question_id
                ),
            );
        }
    ...
    fn process_next_speaker(&mut self, node: &mut DoraNode) -> Result<()> {
        ...
        if let Some(next_speaker) = self.policy.determine_next_speaker() {
            ...
            if cycle > 0 {
                ...
                self.current_question_id =
                    encode_enhanced_question_id(cycle, participant_index, total_participants);
            }
            ...
            send_log(
                ...
                self.log_level,
                &format!(
                    "⏳ Now waiting for session_start for question_id={}",
                    self.current_question_id
                ),
            );
        } else {
        ...
        node.send_output(
            ...
            StringArray::from(vec![
                serde_json::to_string(&self.policy.get_stats())?.as_str()
            ]),
        )?;
    ...
    fn send_cancel_to_all_llms(&self, node: &mut DoraNode) -> Result<()> {
        ...
        send_log(
            ...
            self.log_level,
            &format!(
                "🛑 Sent cancel to all LLMs with question_id={}",
                self.current_question_id
            ),
        );
    ...
    fn send_reset_to_all_bridges(&self, node: &mut DoraNode) -> Result<()> {
        ...
        send_log(
            ...
            self.log_level,
            &format!(
                "🔄 Sent reset to all bridges with question_id={}",
                self.current_question_id
            ),
        );
    ...
    fn send_reset_to_audio_pipeline(&self, node: &mut DoraNode) -> Result<()> {
        ...
        send_log(
            ...
            self.log_level,
            &format!(
                "🔄 Sent reset to audio pipeline with question_id={}",
                self.current_question_id
            ),
        );
    ...
    fn reset_to_initial_state(&mut self, node: &mut DoraNode) -> Result<()> {
        ...
        send_log(
            ...
            self.log_level,
            &format!(
                "✅ Reset complete - ready to start with question_id={} ({})",
                self.current_question_id,
                enhanced_id_debug_string(self.current_question_id)
            ),
        );
    ...
    fn get_stats(&self) -> serde_json::Value {
        ...
        if let serde_json::Value::Object(ref mut map) = stats {
            map.insert(
                ...
                serde_json::Value::Number(self.participant_inputs.len().into()),
            );
            map.insert(
...
fn main() -> Result<()> {
    ...
    send_log(
        ...
        LogLevel::Info,
        log_level,
        &format!("🚀 Controller started with pattern: {}", pattern),
    );
        ...
        match event {
            Some(Event::Input {
                ...
                send_log(
                    ...
                    LogLevel::Debug,
                    log_level,
                    &format!("📨 Received event from input: '{}'", id.as_str()),
                );
                ...
                if id.as_str() == "control" {
                    ...
                    let parsed_json: Option<serde_json::Value> =
                        serde_json::from_str(control_text).ok();
                            ...
                            match command.to_lowercase().as_str() {
                                ...
                                "cancel" => {
                                    ...
                                    send_log(
                                        ...
                                        LogLevel::Info,
                                        log_level,
                                        "🛑 Sent cancel command to all LLMs",
                                    );
                                }
                                "stats" => {
                                ...
                                _ => {
                                    send_log(
                                        ...
                                        LogLevel::Warn,
                                        log_level,
                                        &format!("Unknown JSON command: {}", command),
                                    );
                                }
                            }
                        }
                        ...
                        match control_text.to_lowercase().as_str() {
                            ...
                            "stats" => {
                                ...
                                node.send_output(
                                    ...
                                    Default::default(),
                                    StringArray::from(
                                        vec![serde_json::to_string(&stats)?.as_str()],
                                    ),
                                )?;
                            }
                            _ => {
                    ...
                    send_log(
                        ...
                        LogLevel::Info,
                        log_level,
                        "🎬 Received session_start input from audio player",
                    );
                    ...
                    let question_id = if let Some(Parameter::String(qid_str)) =
                        metadata.parameters.get("question_id")
                    {
                        ...
                        send_log(
                            ...
                            LogLevel::Warn,
                            log_level,
                            "⚠️ Session start signal missing question_id metadata",
                        );
                        continue;
                    };
                    ...
                    if let Err(e) =
                        controller.handle_session_start(question_id, &mut node, log_level)
                    {
                        send_log(
                            &mut node,
                            LogLevel::Error,
                            log_level,
                            &format!("❌ Error handling session start: {}", e),
                        );
                    }
                    ...
                    send_log(
                        ...
                        LogLevel::Debug,
                        log_level,
                        "📊 Received buffer_status (ignored)",
                    );
                } else {
                    ...
                    send_log(
                        ...
                        LogLevel::Debug,
                        log_level,
                        &format!("📨 Processing input from {}", id),
                    );

                    if let Err(e) =
                        controller.handle_participant_input(id.as_str(), text, &metadata, &mut node)
                    {
                        send_log(
                            &mut node,
                            LogLevel::Error,
                            log_level,
                            &format!("❌ Error handling input: {}", e),
                        );
                    }
                }
            }
