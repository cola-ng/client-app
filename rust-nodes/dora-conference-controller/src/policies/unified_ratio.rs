...
impl PatternParser {
    ...
    fn parse_sequential(pattern: &str) -> Result<PolicyPattern, String> {
        ...
        let participants: Vec<String> = cleaned
            .split('→')
    ...
    fn parse_ratio_priority(pattern: &str) -> Result<PolicyPattern, String> {
        ...
        Ok(PolicyPattern::RatioPriority {
            participants,
            weights,
        })
    }
    ...
    fn parse_parenthesized_entry(entry: &str) -> Result<Option<(String, Weight)>, String> {
        ...
        if parts.len() != 2 {
            return Err(format!(
                "Invalid format in '{}': expected (Name, weight)",
                entry
            ));
        }
            ...
            PolicyPattern::RatioPriority {
                ...
                for weight in weights {
                    if let Weight::Ratio(ratio) = weight {
                        if *ratio <= 0.0 {
                            return Err("Positive ratios only".to_string());
                        }
                    }
                }
                Ok(())
...
impl UnifiedRatioPolicy {
    ...
    pub fn configure(&mut self, pattern_str: &str) -> Result<(), String> {
        ...
        match &pattern {
            PolicyPattern::RatioPriority { participants, .. }
            | PolicyPattern::Sequential { participants, .. } => {
    ...
    pub fn get_participants(&self) -> Vec<String> {
        match &self.pattern {
            PolicyPattern::RatioPriority { participants, .. }
            | PolicyPattern::Sequential { participants, .. } => participants.clone(),
        }
    }

    /// Reset word counts
    pub fn reset_counts(&mut self) {
        for count in self.word_counts.values_mut() {
            *count = 0;
        }
        self.position = 0;
        self.last_speaker = None;
    ...
    pub fn get_stats(&self) -> serde_json::Value {
        ...
        match &self.pattern {
            PolicyPattern::RatioPriority {
                ...
                stats.insert(
                    "mode".to_string(),
                    serde_json::Value::String("ratio_priority".to_string()),
                );
                stats.insert(
                ...
                let weight_objects: Vec<_> = participants
                    .iter()
                    .zip(weights)
                    .map(|(p, w)| {
                        ...
                        match w {
                            Weight::Priority => obj.insert(
                                "weight".to_string(),
                                serde_json::Value::String("*".to_string()),
                            ),
                            Weight::Ratio(r) => obj.insert(
                                "weight".to_string(),
                                serde_json::Value::Number(
                                    serde_json::Number::from_f64(*r).unwrap(),
                                ),
                            ),
                        };
                ...
                stats.insert(
                    "weights".to_string(),
                    serde_json::Value::Array(weight_objects),
                );
            }
            PolicyPattern::Sequential {
                ...
                stats.insert(
                    ...
                    serde_json::Value::Bool(*loop_forever),
                );
            }
        }
        let word_count_obj: serde_json::Map<String, serde_json::Value> = self
            .word_counts
            .iter()
            .map(|(k, v)| {
                (
        ...
        stats.insert(
            ...
            serde_json::Value::Object(word_count_obj),
        );
        ...
        if let PolicyPattern::Sequential { participants, .. } = &self.pattern {
            if !participants.is_empty() {
                ...
                stats.insert(
                    ...
                    serde_json::Value::String(next_speaker.clone()),
                );
            }
        }
        stats.insert(
            "cycle".to_string(),
            serde_json::Value::Number(self.get_current_cycle().into()),
        );
...
impl Policy for UnifiedRatioPolicy {
    fn update_word_count(&mut self, speaker: &str, word_count: usize) {
        if let Some(count) = self.word_counts.get_mut(speaker) {
            *count += word_count;
        }
...
impl UnifiedRatioPolicy {
    ...
    fn determine_ratio_or_priority_speaker(
        &mut self,
        participants: &[String],
        weights: &[Weight],
    ) -> Option<String> {
            ...
            match weight {
                Weight::Priority => {
                    if self.last_speaker.as_deref() != Some(participant) {
                        priority_candidates.push((i, participant.clone()));
                    }
                }
                Weight::Ratio(_) => {
                    non_priority_participants.push(participant.clone());
                    non_priority_weights.push(weights[i].clone());
                }
            }
        }
        ...
        if non_priority_participants.is_empty() {
            return None;
        }
        let ratios: Vec<f64> = non_priority_weights
            .iter()
            .map(|w| if let Weight::Ratio(r) = w { *r } else { 0.0 })
            .collect();
        ...
        for participant in &non_priority_participants {
            actual_counts.push(*self.word_counts.get(participant).unwrap_or(&0));
        }
        let total_actual: usize = actual_counts.iter().sum();
        let mut best_speaker_idx = 0;
        let mut best_score = f64::MIN;
        for i in 0..non_priority_participants.len() {
            if non_priority_participants.len() > 1
                && self.last_speaker.as_deref() == Some(&non_priority_participants[i])
            {
                continue;
            }
            let ideal_words = if total_actual == 0 {
                ratios[i] / total_ratio
            } else {
                (ratios[i] / total_ratio) * (total_actual as f64)
            };
            let actual_words = actual_counts[i] as f64;
            let ratio_difference = (ideal_words - actual_words) / ratios[i].max(1.0);
            if ratio_difference > best_score {
                best_score = ratio_difference;
                best_speaker_idx = i;
            }
        }
    ...
    fn determine_sequential_speaker(
        &mut self,
        participants: &[String],
        loop_forever: bool,
    ) -> Option<String> {
        ...
        if self.position >= participants.len() {
            if loop_forever {
                self.position = 0;
                self.sequential_cycle += 1;
            } else {
                self.position = participants.len() - 1;
            }
        }
        self.last_speaker = Some(speaker.clone());
        Some(speaker)
